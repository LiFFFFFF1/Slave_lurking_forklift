...
#define ENCODER_TBF_POS_COMP_L_MM     12    // 左拉绳软件位置补偿，正值让最终反馈位置变大，不保存到24C64
#define ENCODER_TBF_POS_COMP_R_MM      8    // 右拉绳软件位置补偿，正值让最终反馈位置变大，不保存到24C64
    ...
    switch(RxMessage->StdId)
    {
        ...
        case 0x180+TBF_CAN_ID+1:
            ...
            if(RxMessage->DLC == 0x04)
            {
                ...
                // 软件补偿只改变运行坐标，不改24C64里的硬零位；伸缩控制和上传反馈都会使用补偿后的值
