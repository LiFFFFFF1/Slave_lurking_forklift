...
#define NEW_UP_FRAME_LENGTH   24  // 上传帧长度
...
void PLC_Hub_CustomTX_Mesg_Ask(void)
{
    ...
    upload_frame[14] = EM_UNIT_ACT_ID_ALL;                    // 0-左右叉齿同步快照
    upload_frame[15] = GET_LOW_BYTE(Motor_Lift_Stru.unit_act_data[0]);  // 左叉尺位置L
    upload_frame[16] = GET_HIGH_BYTE(Motor_Lift_Stru.unit_act_data[0]); // 左叉尺位置H
    upload_frame[17] = GET_LOW_BYTE(Motor_Lift_Stru.unit_err_code[0]);  // 左叉齿故障码L
    upload_frame[18] = GET_HIGH_BYTE(Motor_Lift_Stru.unit_err_code[0]); // 左叉齿故障码H
    upload_frame[19] = GET_LOW_BYTE(Motor_Lift_Stru.unit_act_data[1]);  // 右叉尺位置L
    upload_frame[20] = GET_HIGH_BYTE(Motor_Lift_Stru.unit_act_data[1]); // 右叉尺位置H
    upload_frame[21] = GET_LOW_BYTE(Motor_Lift_Stru.unit_err_code[1]);  // 右叉齿故障码L
    upload_frame[22] = GET_HIGH_BYTE(Motor_Lift_Stru.unit_err_code[1]); // 右叉齿故障码H
